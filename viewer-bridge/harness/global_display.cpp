...
void path(const std::vector<uint8_t>&b,uint32_t es,uint32_t ef,uint32_t el){
 ...
 ok(id&&s==es&&f==ef&&l==el&&(flags&~15U)==0&&cc&&ci<cc&&n&&n<=4096&&model&&cont,"bad path header");uint32_t pr=0,pt=0;
 for(uint16_t i=0;i<n;++i){uint32_t rel=r.d(),time=r.d();uint8_t seg=r.b();r.z(1);r.s(18);ok((i==0||(rel>pr&&time>pt))&&rel<=LAST&&time==rel*2048U&&seg>=1&&seg<=5,"bad path point");pr=rel;pt=time;}r.end();
}
...
void report(const std::filesystem::path&o,const std::filesystem::path&dll,const std::filesystem::path&man,const std::string&commit,const std::string&catalog,const std::string&sem,const Times&av,const Times&ra,const Times&pa,size_t paths,uint8_t terminal,const std::array<S,IMPORTANT.size()>&milestones){
 ...
 f<<"{\n  \"schema\":\"ksa64.phase12c.global-display-harness.v1\",\n  \"pass\":true,\n"
  ...
  <<"  \"nominal_replay\":{\"samples\":22015,\"first_release\":0,\"last_release\":22014,\"transitions\":4,\"director_source_mask\":11,\"guided_source_mask\":3,\"successful_path_requests\":"<<paths<<",\"terminal_disposition\":"<<unsigned(terminal)<<",\"semantic_sha256\":\""<<sem<<"\"},\n"
  <<"  \"milestones\":[";
...
int main(int argc,char**argv){try{
 ...
 Times av,ra,pa;auto*dir=start(api,DIR);try{
  ...
  auto b=out();auto def=take(api.definition_payload(dir,&b),b,free);definition(def,DIR_MASK);b=out();auto idx=take(api.replay_index_payload(dir,&b),b,free);std::array<bool,IMPORTANT.size()>ih{};auto terminal=index(idx,ih);std::array<bool,IMPORTANT.size()>sh{};std::array<S,IMPORTANT.size()>milestones{};std::vector<uint8_t>sem;sem.insert(sem.end(),def.begin(),def.end());sem.insert(sem.end(),idx.begin(),idx.end());uint32_t expected=0;
  for(uint32_t startrel=0;startrel<N;startrel+=256){Ksa64GlobalDisplaySampleRangeRequestV1 q{};q.api_version=1;q.struct_size=sizeof(q);q.start_release=startrel;q.max_count=std::min<uint32_t>(256,N-startrel);b=out();auto begun=std::chrono::steady_clock::now();auto code=api.sample_range_payload(dir,&q,&b);ra.add(std::chrono::steady_clock::now()-begun);auto payload=take(code,b,free);auto ss=samples(payload,DIR_MASK);ok(ss.size()==q.max_count,"range sample count mismatch");for(auto s:ss){ok(s.release==expected++,"exact release ordering mismatch");for(size_t j=0;j<IMPORTANT.size();++j){if(s.release==IMPORTANT[j]){sh[j]=true;milestones[j]=s;}}}sem.insert(sem.end(),payload.begin(),payload.end());}
  ok(expected==N&&std::all_of(ih.begin(),ih.end(),[](bool x){return x;})&&std::all_of(sh.begin(),sh.end(),[](bool x){return x;}),"important release omitted");for(auto er:std::array<uint32_t,4>{29,3579,12669,15255}){b=out();auto p=take(api.poll_transition_payload(dir,&b),b,free);ok(transition(p)==er,"transition mismatch");sem.insert(sem.end(),p.begin(),p.end());}b=out();ok(api.poll_transition_payload(dir,&b)==KSA64_VIEWER_NO_DATA&&!b.data&&!b.length,"unexpected transition");
  size_t pc=0;for(uint32_t source:std::array<uint32_t,3>{1,2,4}){bool found=false;for(uint32_t frame=1;frame<=3;++frame){Ksa64GlobalDisplayPathRequestV1 q{};q.api_version=1;q.struct_size=sizeof(q);q.source=source;q.display_frame=frame;q.lod=source==1?2:3;b=out();auto begun=std::chrono::steady_clock::now();auto code=api.path_chunk_payload(dir,&q,&b);pa.add(std::chrono::steady_clock::now()-begun);if(code==KSA64_VIEWER_NO_DATA){ok(!b.data&&!b.length,"path no-data allocated");continue;}auto p=take(code,b,free);path(p,source,frame,q.lod);sem.insert(sem.end(),p.begin(),p.end());found=true;++pc;}ok(found,"permitted source has no path");}
  ...
  report(output,dll,man,commit,catalog,hex(ksa64::crypto::sha256(sem.data(),sem.size())),av,ra,pa,pc,terminal,milestones);ok(destroy(dir)==0,"director destroy failed");dir=nullptr;
