...
int main(int argc,char**argv){
 ...
 const char invalid_role_text[]="unknown-role";auto invalid_role=make_span(reinterpret_cast<const uint8_t*>(invalid_role_text),sizeof(invalid_role_text)-1);auto*rejected_handle=reinterpret_cast<Ksa64ViewerHandle*>(static_cast<uintptr_t>(1));assert(a.start(&invalid_role,&rejected_handle)==KSA64_VIEWER_UNSUPPORTED&&rejected_handle==nullptr);
 ...
 constexpr size_t kPollSamples=4096;auto poll_begin=std::chrono::steady_clock::now();std::chrono::steady_clock::duration poll_max{};for(size_t i=0;i<kPollSamples;++i){auto sample=snap_header();auto begin=std::chrono::steady_clock::now();assert(a.poll(h,&sample)==KSA64_VIEWER_UNCHANGED);auto elapsed=std::chrono::steady_clock::now()-begin;if(elapsed>poll_max)poll_max=elapsed;}auto poll_total=std::chrono::steady_clock::now()-poll_begin;assert(poll_total<std::chrono::seconds(5));assert(poll_max<std::chrono::seconds(1));
