...
int main(void) {
    VerticalRun run;
    run_vertical_workload_optimized(run);
...
int main(void) {
    ...
    if (failures == 0) {
        printf(
            "Phase 0 optimized vertical workload: PASS (checksum %08lx)\n",
            run.checksum
        );
        return 0;
    }
    printf(
        "Phase 0 optimized vertical workload: %u failure(s), checksum %08lx\n",
        failures,
        run.checksum
    );
    return 1;
