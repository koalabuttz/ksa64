...
use ksa64_flight::phase6_realtime::{RealtimeFlightComputer, PAL_TICK_BUDGET_CYCLES};
...
pub extern "C" fn main() -> isize {
    ...
    flight.set_guidance_segment([0, -8066, 0], [0, -8000, 0], [0; 3]);
    unsafe { c64_timer::start_cia_timer() };
    let worst = flight.tick(Some(inertial(0)), Some(aid()));
    let worst_cycles = unsafe { c64_timer::stop_cia_timer() }.wrapping_sub(overhead);
    unsafe { c64_timer::start_cia_timer() };
    let fast = flight.tick(Some(inertial(1)), None);
    let fast_cycles = unsafe { c64_timer::stop_cia_timer() }.wrapping_sub(overhead);
    let mut status = 0;
    if worst.safe || fast.safe {
        status |= 1
    }
    if worst.command.effective_epoch != 1 || fast.command.effective_epoch != 2 {
        status |= 2
    }
    unsafe {
        u16o(4, 1);
        u16o(6, status);
        u32o(8, overhead);
        u32o(12, worst_cycles);
        u32o(16, fast_cycles);
        u32o(20, PAL_TICK_BUDGET_CYCLES);
        u32o(24, flight.navigation().checksum);
        u32o(28, flight.flight_checksum());
