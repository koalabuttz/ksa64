//! Transport-neutral append-only KRA4 archives.

use ksa64_interface::crc32_ieee;

use super::contracts::{ARCHIVE_RECORD_HEADER_LENGTH, ARCHIVE_SUPERBLOCK_LENGTH, KRA4_MAGIC};
use super::PHASE4_CONTRACT_ID;

pub const KRA4_VERSION: u16 = 4;
...
impl<S: ArchiveStorage> ArchiveWriter<S> {
    ...
    fn write_superblock(&mut self, complete: bool) -> Result<(), ArchiveError> {
        ...
        block[0..4].copy_from_slice(&KRA4_MAGIC);
        put_u16(&mut block, 4, KRA4_VERSION);
        put_u16(&mut block, 6, ARCHIVE_SUPERBLOCK_LENGTH as u16);
        put_u32(&mut block, 8, PHASE4_CONTRACT_ID);
...
pub fn scan_archive<S: ArchiveStorage>(storage: &mut S) -> Result<ArchiveScan, ArchiveError> {
    ...
    storage
        .read(0, &mut block)
        .map_err(|_| ArchiveError::Storage)?;
    if block[0..4] != KRA4_MAGIC {
        return Err(ArchiveError::Magic);
    }
    if get_u16(&block, 4) != KRA4_VERSION || get_u16(&block, 6) != ARCHIVE_SUPERBLOCK_LENGTH as u16
    {
        return Err(ArchiveError::Version);
    }
    if get_u32(&block, 8) != PHASE4_CONTRACT_ID {
