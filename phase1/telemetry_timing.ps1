...
$artifact = Join-Path $projectRoot (
    "target\mos-c64-none\release\ksa64-phase1-telemetry-timed-c64"
)
...
try {
    ...
    Write-Host "== Phase 1 telemetry timing build =="
    & $rustWrapper -ReturnToCaller -WorkingDirectory "." `
        cargo build --release --target mos-c64-none --features c64 `
        -Z build-std=core `
        -Z build-std-features=compiler-builtins-mem `
        --bin ksa64-phase1-telemetry-timed-c64
    Assert-ExitCode "Telemetry timing build"

    Write-Host ""
    Write-Host "== PAL VICE $($versions.vice.version), CIA1 32-bit timer =="
    $json = & python -B phase1/reference/vice_telemetry_timing.py `
        --vice $vice `
        --prg $artifact `
        --runs $Runs `
        --timeout 300
    Assert-ExitCode "Telemetry VICE timing"
    ...
    $rows = @(
        ...
        [pscustomobject]@{
            Path = "Checksum + telemetry"
            Cycles = [long]$result.telemetry_net_cycles
            CyclesPerStep = "{0:N2}" -f [double]$result.telemetry_cycles_per_step
            MarginAt8Hz = "{0:N2}" -f ($budgetPerStep - [double]$result.telemetry_cycles_per_step)
        }
    )
    ...
    Write-Host ("Telemetry delta: {0:N0} cycles ({1:N2}/step, {2:N2}/frame)." -f `
        [long]$result.telemetry_overhead_cycles, `
        [double]$result.telemetry_overhead_per_step, `
