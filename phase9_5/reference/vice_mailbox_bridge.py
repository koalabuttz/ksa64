...
def capabilities()->bytes:
    ...
    struct.pack_into('<III',out,4,13,0x06010001,0x4587892C)
    struct.pack_into('<I',out,16,0x09520001)
    struct.pack_into('<HBBB',out,20,512,32,8,1)
    return bytes(out)

def main()->int:
    parser=argparse.ArgumentParser(description='One-instance externally paced Phase 9.5 C64-flight probe')
    parser.add_argument('--vice',type=Path,required=True);parser.add_argument('--prg',type=Path,required=True)
    parser.add_argument('--broker',type=Path,required=True);parser.add_argument('--max-releases',type=int,default=8)
    ...
    vice=args.vice.resolve(strict=True);prg=args.prg.resolve(strict=True);broker=args.broker.resolve(strict=True)
    ...
    print(f'VICE Phase 9.5 split probe PID {vp.pid}; one instance; warp disabled; externally paced; finite {args.max_releases} releases',flush=True)
    try:
        ...
        broker_command=[str(broker),'--listen',f'127.0.0.1:{broker_port}','--pace','externally-paced','--max-releases',str(args.max_releases)]
        ...
        expected=f'KSA64_PHASE95_BOUNDED releases={completed}'
        ...
        result={'schema':'ksa64.phase9_5.vice-mailbox-probe-v1','releases':completed,'wall_seconds':time.monotonic()-started,'target':'PAL stock C64 flight endpoint via one pinned VICE instance; externally paced KLF6/KLR9 step-and-ack, not realtime','artifact':{'bytes':len(raw_prg),'sha256':hashlib.sha256(raw_prg).hexdigest(),'load_address':load,'load_end_exclusive':end,'stock_fit':end<=0xC000},'broker':output.strip(),'result_raw':raw.hex()}
