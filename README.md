...
> **Project status:** Phases 0–12B are complete. Phase 12B accepted a packaged, role-filtered KSA-G10R GNSS-loss operations desk whose exact four-action mission completes at release 21,591 (674.71875 seconds). The run achieves its primary objective and completes nominal vehicle recovery with complete evidence; persistent GNSS loss leaves avionics degraded, so the aggregate result is correctly **Degraded Success** rather than Nominal Success. Unreal Engine 5.8 automation passed 17/17, the packaged D3D12 presentation preserved the exact 2,911,464-byte KSB11 archive, and its bounded bridge/presentation workload stayed below the accepted latency budget. Phase 12B.5 is active under its frozen implementation contract and is building the cross-platform runtime, bridge, web/PWA, Vita/SDL2, and portable-session foundation before the global 3-D viewer. The accepted missions, workbenches, evidence, and C64 targets remain discoverable through one `ksa64` application and deterministic product catalog. Realtime C64 flight, a 6502-specific rewrite, C64 Ultimate acceleration, physical-link acceptance, and the portable C64 world remain parallel priority tracks.
...
|---|---|
| 12B.5 — Platform foundation | **Active:** portable session authority and KPS1 presentation protocol, Windows/Linux/macOS and ARM64 parity, an 8 GB Duet lane, compact React/Babylon PWA with exact Rust/WASM authority, and Vita/SDL2. |
