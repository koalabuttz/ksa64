...
> Project status: Phase 1 implementation. Raw physics clears 8 Hz; canonical checksummed telemetry is measured at 5.72 Hz through an allocation-free discard sink. Host capture and inspection are next.
...
Phase 0 is complete, and the Phase 1 exact core executes the full golden vertical mission while preserving the last valid state on faults. Two exact fast paths save 45,923.05 cycles per step, and three common-clock runs measure checked dynamics at 114,981.59 PAL cycles per step—8.57 Hz with 6.64 percent raw headroom. Canonical mission-stream emission matches the independent 257-frame schedule and byte-stream CRC across native and MOS targets. Three stable PAL measurements show telemetry serialization and discard delivery add 7,475.28 cycles per step over checksum mode; the complete recorded-validation path reaches 5.72 Hz. The next milestone is a host capture and inspection adapter using the same sink boundary.
