...
> **Project status:** Phases 0–12B are complete. Phase 12B accepted a packaged, role-filtered KSA-G10R GNSS-loss operations desk whose exact four-action mission completes at release 21,591 (674.71875 seconds). The run achieves its primary objective and completes nominal vehicle recovery with complete evidence; persistent GNSS loss leaves avionics degraded, so the aggregate result is correctly **Degraded Success** rather than Nominal Success. Unreal Engine 5.8 automation passed 17/17, the packaged D3D12 presentation preserved the exact 2,911,464-byte KSB11 archive, and its bounded bridge/presentation workload stayed below the accepted latency budget. Phase 12B.5 software implementation and local acceptance are complete: the portable session and KPS1 boundary, cross-platform bridge, secure broker, compact web/PWA, exact Rust/WASM authority, and Vita/SDL2 client are built. The complete hosted Windows x64, Linux x64, Linux ARM64, and macOS ARM64 matrices plus WASM worker exactness under the Node harness now pass; full phase acceptance remains pending the physical Lenovo Duet, Vita3K, and physical Vita gates. Phase 12C is under completion audit: the renderer-neutral global display, exact replay, Unreal, Babylon, and strict v2 parity-evidence pipeline are implemented and hardened through complete source/path/event/continuity semantics and one normalized shared view-mode policy, but final packaged renderer parity and measured runtime-performance acceptance remain explicitly pending. The accepted missions, workbenches, evidence, and C64 targets remain discoverable through one `ksa64` application and deterministic product catalog. Realtime C64 flight, a 6502-specific rewrite, C64 Ultimate acceleration, physical-link acceptance, and the portable C64 world remain parallel priority tracks.
...
- an accepted Phase 12B full GNSS-loss operations lane whose four-action scripted reference completes at release 21,591, seals 2,911,464 bytes of exact KSB11 evidence, and records **Degraded Success** across independent mission, vehicle, procedure, operator, avionics, and evidence axes; the packaged 1920x1080 D3D12 desk passed 17/17 automation tests with zero queue overflow, 258.9 microseconds p99, and 460 microseconds maximum bridge/presentation service time.
- the Phase 12B.5 portable session and `no_std + alloc` KPS1 presentation crates, strict Rust-owned KSB11 replay, portable ABI-v1 libraries, loopback PWA broker, Noise-paired native/Vita LAN, compact React/Babylon Mission Control PWA, byte-exact single-worker Rust/WASM authority, and a bounded Vita/SDL2 VPK; hosted portable-runtime qualification is complete; physical Duet, Vita3K emulator, and physical Vita qualification remain explicitly pending.
- an in-progress Phase 12C `GlobalDisplayV1` boundary with additive KPS1/ABI discovery, Rust-owned ENU/ECEF/GCRF display products, exact nominal/GNSS replay, passive procedural Unreal/Babylon viewers, temporal/event-aware path checksums, raw path-state flags, normalized view modes, and a strict `ksa64.phase12c.cross-renderer-evidence.v2` pipeline; completion remains conditional on the explicit packaged, rendered-browser, cross-renderer, and measured-performance gates.
...
| 12B — Live operations | Complete: accepted 674.71875-second role-filtered GNSS-loss operations mission, exact KSB11/KTT10/KPH10/KSR10 evidence, 17/17 Unreal automation, standalone packaged execution, accessible 2-D command desk, and bounded presentation-service timing. |

## Active and planned phases

| Phase | Intended result |
|---|---|
| 12B.5 — Platform foundation | **Software- and hosted-portable-runtime-qualified; device acceptance pending:** portable session/KPS1 authority, cross-platform bridge and packaging, secure broker, React/Babylon PWA with exact Rust/WASM authority, and Vita/SDL2 are implemented; the required Duet, Vita3K, and physical Vita evidence remains open. |
| 12C — Global engineering viewer | **Implementation hardened; completion audit in progress:** renderer-neutral Earth-scale 3-D visualization through desktop Unreal and WebGPU/WebGL browser lanes, with exact ENU/ECEF/GCRF display domains, events, entry, recovery, strict source-bound parity evidence, and explicit pending packaged/runtime measurements. |
| 12C.5 — Portable operations | Polished web/PWA, Vita/SDL2, Android, and iOS Mission Control, replay, actions, and scaled 3-D clients with measured optional local execution. |
...
- [Phase 12](phase12/README.md), [12A implementation contract](phase12/PLAN.md), [completion record](phase12/COMPLETION.md), [completion audit](phase12/complete.ps1), [historical Phase 12B handoff](phase12/PHASE12B_HANDOFF.md), [12B implementation contract](phase12/PHASE12B_PLAN.md), [accepted 12B completion record](phase12/PHASE12B_COMPLETION.md), [12B audit](phase12/complete-phase12b.ps1), [Phase 12B.5 handoff](phase12/PHASE12B5_HANDOFF.md), [12B.5 implementation contract](phase12/PHASE12B5_PLAN.md), [12B.5 implementation checkpoint](phase12/PHASE12B5_COMPLETION.md), [12B.5 CI matrix](phase12/CI.md), [web platform decision](phase12/WEB_PLATFORM.md), [Phase 12C plan](phase12/PHASE12C_PLAN.md), [nominal compatibility decision](phase12/PHASE12C_NOMINAL_COMPATIBILITY.md), [draft Phase 12C completion ledger](phase12/PHASE12C_COMPLETION.md), [Phase 12C audit](phase12/complete-phase12c.ps1), [draft Phase 12C.5 handoff](phase12/PHASE12C5_HANDOFF.md), [draft Phase 12D handoff](phase12/PHASE12D_HANDOFF.md), [engine decision](phase12/ENGINE_DECISION.md), [toolchain lock procedure](phase12/TOOLCHAIN.md), [bridge contract](phase12/BRIDGE.md), and [asset policy](phase12/ASSETS.md)
...
The Phase 12C portable/contracts audit is:

```powershell
pwsh -File phase12/complete-phase12c.ps1
```

Its default invocation validates frozen entry evidence, formatting, Clippy, native contracts, exact nominal and GNSS replays, the bridge/harness, broker, WebAssembly, and the production web build. It deliberately does **not** claim Phase 12C completion. A completion pass additionally requires explicit Unreal build, automation, package, rendered-browser, semantic-parity, and runtime-performance evidence switches and manifests. The strict joined record uses `ksa64.phase12c.cross-renderer-evidence.v2`. For UE 5.8 Launcher binary compatibility, project modules compile with `RayTracingMode=Inline` while runtime ray tracing remains disabled with `r.RayTracing=False`; this does not create a ray-tracing hardware or visual-feature requirement.
