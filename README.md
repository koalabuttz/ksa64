...
> **Project status:** Phases 0–12B are complete. Phase 12B accepted a packaged, role-filtered KSA-G10R GNSS-loss operations desk whose exact four-action mission completes at release 21,591 (674.71875 seconds). The run achieves its primary objective and completes nominal vehicle recovery with complete evidence; persistent GNSS loss leaves avionics degraded, so the aggregate result is correctly **Degraded Success** rather than Nominal Success. Unreal Engine 5.8 automation passed 17/17, the packaged D3D12 presentation preserved the exact 2,911,464-byte KSB11 archive, and its bounded bridge/presentation workload stayed below the accepted latency budget. Phase 12C is next and owns the passive global 3-D engineering viewer. The accepted missions, workbenches, evidence, and C64 targets remain discoverable through one `ksa64` application and deterministic product catalog. Realtime C64 flight, a 6502-specific rewrite, C64 Ultimate acceleration, physical-link acceptance, and the portable C64 world remain parallel priority tracks.
...
|---|---|
| 12C — Global engineering viewer | Next: passive Earth-scale 3-D visualization of the accepted global mission, including ENU/ECEF/GCRF display domains, Earth, vehicle pose, exact events, entry, recovery, cameras, and packaged performance. |
