...
> Project status: Phase 1 implementation. Raw physics clears 8 Hz; canonical telemetry is captured and strictly inspected on the host, while its complete C64 validation mode is measured at 5.72 Hz. A C64 text-status sink is next.
...
Phase 0 is complete, and the Phase 1 exact core executes the full golden vertical mission while preserving the last valid state on faults. Two exact fast paths save 45,923.05 cycles per step, and three common-clock runs measure checked dynamics at 114,981.59 PAL cycles per step—8.57 Hz with 6.64 percent raw headroom. Canonical mission-stream emission matches the independent 257-frame schedule and byte-stream CRC across native and MOS targets. Three stable PAL measurements show telemetry serialization and discard delivery add 7,475.28 cycles per step over checksum mode; the complete recorded-validation path reaches 5.72 Hz. The host adapter now captures and strictly validates that stream through the same sink boundary. The next milestone is a C64 text-status sink that retains only the latest accepted frame.
