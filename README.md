...
> **Project status:** Phases 0–12B are complete. Phase 12B accepted a packaged, role-filtered KSA-G10R GNSS-loss operations desk whose exact four-action mission completes at release 21,591 (674.71875 seconds). The run achieves its primary objective and completes nominal vehicle recovery with complete evidence; persistent GNSS loss leaves avionics degraded, so the aggregate result is correctly **Degraded Success** rather than Nominal Success. Unreal Engine 5.8 automation passed 17/17, the packaged D3D12 presentation preserved the exact 2,911,464-byte KSB11 archive, and its bounded bridge/presentation workload stayed below the accepted latency budget. Phase 12B.5 software implementation and local acceptance are complete: the portable session and KPS1 boundary, cross-platform bridge, secure broker, compact web/PWA, exact Rust/WASM authority, and Vita/SDL2 client are built. The complete hosted Windows x64, Linux x64, Linux ARM64, and macOS ARM64 matrices plus WASM worker exactness under the Node harness now pass; full phase acceptance remains pending the physical Lenovo Duet, Vita3K, and physical Vita gates. Phase 12C is under completion audit: the renderer-neutral global display, exact replay, Unreal, and Babylon implementation is present, but packaged renderer parity and runtime-performance acceptance remain explicitly pending. The accepted missions, workbenches, evidence, and C64 targets remain discoverable through one `ksa64` application and deterministic product catalog. Realtime C64 flight, a 6502-specific rewrite, C64 Ultimate acceleration, physical-link acceptance, and the portable C64 world remain parallel priority tracks.
...
- an in-progress Phase 12C `GlobalDisplayV1` boundary with additive KPS1/ABI discovery, Rust-owned ENU/ECEF/GCRF display products, exact nominal/GNSS replay, and passive procedural Unreal/Babylon viewers; completion remains conditional on the explicit packaged, rendered-browser, cross-renderer, and performance gates.
...
| 12C — Global engineering viewer | **Implementation under completion audit:** renderer-neutral Earth-scale 3-D visualization through desktop Unreal and WebGPU/WebGL browser lanes, with exact ENU/ECEF/GCRF display domains, events, entry, recovery, and explicit pending packaged/runtime parity gates. |
