...
Status: accepted. The powered KSA-2A mission satisfies the declared accuracy envelope, so semi-implicit Euler remains the product integrator.
...
Three stable final-layout PAL VICE measurements record 451,562.59 cycles per semi-implicit step and 452,574.37 cycles per midpoint step. The planar vacuum kernel is intentionally accuracy-first and is already slower than real time; no throughput floor applies. The result selects semi-implicit Euler because RK2 currently adds code/evaluation structure without changing accepted product values.

`vacuum-timing-v2.json` freezes the final linked target measurement; the earlier v1 snapshot remains layout-history evidence. `integrator-v1.json` independently evaluates the same elliptical coast in floating point, including RK4 at 1/32 and 1/64 of the product timestep. A future model should reopen the choice if added fidelity exceeds the accepted insertion or coast tolerances.
