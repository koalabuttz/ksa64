...
contained-panic tests. The web job activates only after
`web/package-lock.json` exists, then uses `npm ci`, tests, and a production
build.
...
The WebAssembly acceptance job is added when the Worker adapter can execute
the same session. It is not replaced by a TypeScript mock.
...
The current hosted-runner labels and architectures are recorded from the
