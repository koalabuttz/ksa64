...
impl FullMissionSession {
    ...
    /// Immutable compiled session-definition identity exposed to presentation clients.
    pub const fn definition_identity(&self) -> u32 {
        self.project.definition_identity
    }

    /// Identity of the currently staged command load, if any.
    pub fn staged_load_identity(&self) -> Option<u32> {
        self.staged_load.map(|load| load.load_identity)
    }

    /// Current deterministic action-transcript chain.
    pub const fn action_chain(&self) -> u32 {
        self.transcript.chain
    }

    /// Current procedure-evidence chain, or zero before procedure entry.
    pub fn procedure_chain(&self) -> u32 {
        self.procedure
            .as_ref()
            .and_then(|procedure| procedure.evidence().last())
            .map_or(0, |record| record.chain)
    }

    /// Current flight-package event-journal chain, or zero before preparation.
    pub fn journal_chain(&self) -> u32 {
        ...
        records[..count].last().map_or(0, |record| record.chain)
    }

    /// Current procedure state, preserving the accepted procedure engine's semantics.
    pub fn procedure_state(&self) -> Option<ProcedureState> {
        self.procedure.as_ref().map(ProcedureEngine::state)
    }

    /// Borrow sealed evidence without consuming the live session.
    pub fn completed_session(&self) -> Option<&FullMissionCompletion> {
