...
## Mission Control visualization addendum

The host-native console now uses the complete passive update history to provide responsive Ascent, Orbit, and Ground Track views, planned-versus-observed overlays, operational trend panels, and an ultra-wide three-preview layout. The plan is a strictly validated embedded copy of the frozen 99-point KPH5 baseline. Onboard and delayed/noisy ground estimates remain separate, derived quantities are labelled MODEL EST, and omniscient truth remains exclusive to F7.

This addendum changes no KLR6/KMR6/KLF6/KST5 contract, target image, scheduler, checksum, memory budget, or command path. Acceptance renders every page at five terminal sizes, exercises all three trajectory views in both Braille and strict ASCII modes, checks orbital/geographic edge cases, and proves F1-F6 truth isolation by mutation. A finite one-VICE realtime TUI smoke rendered Ground Track, shadow-verified all eight released cells, and ended with zero deadlines or alarms and a clean process lifecycle. The detailed operator and provenance guide is [MISSION_CONTROL.md](MISSION_CONTROL.md).
